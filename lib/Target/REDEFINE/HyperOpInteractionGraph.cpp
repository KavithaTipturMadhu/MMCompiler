...
//Compute and set dominance frontier
	for (list<HyperOp*>::iterator vertexIterator = Vertices.begin(); vertexIterator != Vertices.end(); vertexIterator++) {
		HyperOp* vertex = *vertexIterator;
		errs() << "finding domf of vertex " << vertex->asString() << "\n";
		...
		for (list<HyperOp*>::iterator dominatedNodeIterator = Vertices.begin(); dominatedNodeIterator != Vertices.end(); dominatedNodeIterator++) {
			...
			if (std::find(dominators.begin(), dominators.end(), vertex) != dominators.end()) {
				errs() << "vertex dominates " << (*dominatedNodeIterator)->asString() << "\n";
				...
				for (list<HyperOp*>::iterator childItr = successorList.begin(); childItr != successorList.end(); childItr++) {
					...
					if ((*childItr == vertex || std::find(dominatorOfChild.begin(), dominatorOfChild.end(), vertex) == dominatorOfChild.end()) && find(dominanceFrontier.begin(), dominanceFrontier.end(), *childItr) == dominanceFrontier.end()) {
						...
						errs() << "added to domf:" << (*childItr)->asString() << "\n";
					}
				}
			}
		}
			...
			for (map<HyperOp*, HyperOp*>::iterator edgeItr = edgeForFix.second.begin(); edgeItr != edgeForFix.second.end(); edgeItr++) {
				...
				for (auto childEdge : edgeSource->ChildMap) {
					if (childEdge.second == edgeDestination && (childEdge.first->getType() == HyperOpEdge::LOCAL_REFERENCE || childEdge.first->getType() == HyperOpEdge::SCALAR)) {
						...
						Argument* arg = new Argument(childEdge.first->getValue()->getType(), StringRef(""), supposedDomHop->getFunction());
						vector<Type*> newArgList;
						for (int i = 0; i < supposedDomHop->getFunction()->getFunctionType()->getNumParams(); i++) {
							newArgList.push_back(supposedDomHop->getFunction()->getFunctionType()->getParamType(i));
						}
						newArgList.push_back(arg->getType());
						FunctionType* functionType = supposedDomHop->getFunction()->getFunctionType();
						*functionType = *(FunctionType::get(Type::getVoidTy(getGlobalContext()), newArgList, false));
						errs()<<"different pointer?"<<functionType<<" and "<<supposedDomHop->getFunction()->getFunctionType()<<"\n";
						supposedDomHop->getFunction()->setFunctionType(functionType);
						errs()<<"updated function type:"<< supposedDomHop->getFunction()->getFunctionType()<<"\n";
