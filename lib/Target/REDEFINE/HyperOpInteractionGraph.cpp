 ...
 */
//TODO same code in 3 places! This is plain horrible!
void HyperOpInteractionGraph::addContextFrameAddressForwardingEdges() {
	Module* parentModule = this->Vertices.front()->getFunction()->getParent();
...
//						&& !(*tempItr)->isStaticHyperOp() && ) {
				...
				if (dominanceFrontierHyperOp->getImmediateDominator() != vertex->getImmediateDominator()) {
					...
					while (immediateDominator != 0 && !immediateDominatorDominanceFrontier.empty() && std::find(immediateDominatorDominanceFrontier.begin(), immediateDominatorDominanceFrontier.end(), dominanceFrontierHyperOp) != immediateDominatorDominanceFrontier.end()) {
						...
						for (map<HyperOpEdge*, HyperOp*>::iterator childMapItr = immediateDominator->ChildMap.begin(); childMapItr != immediateDominator->ChildMap.end(); childMapItr++) {
							if ((childMapItr->first->getType() == HyperOpEdge::CONTEXT_FRAME_ADDRESS_SCALAR || childMapItr->first->getType() == HyperOpEdge::CONTEXT_FRAME_ADDRESS_LOCALREF) && childMapItr->first->getContextFrameAddress() == dominanceFrontierHyperOp
									&& childMapItr->second == prevVertex) {
	...
	//Shuffle the address forward slots for dynamic instances to ensure that dynamic hyperOps have the same context frame layout as the static ones
...
//Important: Works only on a CFG and not HIG nodes
bool mutuallyExclusiveHyperOps(HyperOp* firstHyperOp, HyperOp* secondHyperOp) {
	if (firstHyperOp == 0 || secondHyperOp == 0) {
		return false;
	}
	errs() << "finding mutually exclusive paths between " << firstHyperOp->asString() << " and " << secondHyperOp->asString() << "\n";
	...
	if (firstHyperOp->getImmediateDominator() == secondHyperOp->getImmediateDominator()) {
		if (find(firstHyperOpDomf.begin(), firstHyperOpDomf.end(), secondHyperOp) == firstHyperOpDomf.end() && find(secondHyperOpDomf.begin(), secondHyperOpDomf.end(), firstHyperOp) == secondHyperOpDomf.end()) {
			...
			list<list<pair<HyperOpEdge*, HyperOp*> > > tempPredicateChains = mergePredicateChains(predicateChains);
			if (tempPredicateChains.size() > 1) {
				return false;
			}

			if (firstHyperOpPredicateChain.size() != secondHyperOpPredicateChain.size()) {
				return false;
			}
			//check if the two predicate chains are the same
			auto secondPredChainItr = secondHyperOpPredicateChain.begin();
			for (auto firstPredChainItr = firstHyperOpPredicateChain.begin(); firstPredChainItr != firstHyperOpPredicateChain.end() && secondPredChainItr != secondHyperOpPredicateChain.end(); firstPredChainItr++, secondPredChainItr++) {
				if (firstPredChainItr->first != secondPredChainItr->first || firstPredChainItr->second != secondPredChainItr->second) {
					return false;
				}
			}
	...
	while (true) {
		...
		for (list<HyperOp*>::iterator targetClusterIterator = targetCluster.begin(); targetClusterIterator != targetCluster.end(); targetClusterIterator++) {
			...
			if (sourceClusterNodeFirst != 0) {
				...
				edge->setType(HyperOpEdge::ORDERING);
				edge->setIsEdgeIgnored(true);
				errs() << "added ordering edge between " << sourceClusterNodeFirst->asString() << " and " << targetNodeForMerge->asString() << " that are mutually exclusive?"
						<< mutuallyExclusiveHyperOps(controlFlowGraphAndOriginalHopMap.second[sourceClusterNodeFirst], controlFlowGraphAndOriginalHopMap.second[targetNodeForMerge]) << "\n";
				additionalEdgesMap.push_back(std::make_pair((HyperOpEdge*) edge, make_pair(sourceClusterNodeFirst, targetNodeForMerge)));
				sourceClusterNodeFirst->addChildEdge((HyperOpEdge*) edge, targetNodeForMerge);
				targetNodeForMerge->addParentEdge((HyperOpEdge*) edge, sourceClusterNodeFirst);
			}
			if (sourceClusterNodeSecond != 0) {
				...
				edge->setType(HyperOpEdge::ORDERING);
				edge->setIsEdgeIgnored(true);
				errs() << "added ordering edge between " << sourceClusterNodeSecond->asString() << " and " << targetNodeForMerge->asString() << " that are mutually exclusive?"
						<< mutuallyExclusiveHyperOps(controlFlowGraphAndOriginalHopMap.second[sourceClusterNodeSecond], controlFlowGraphAndOriginalHopMap.second[targetNodeForMerge]) << "\n";
...
//	}
	...
	DEBUG(dbgs() << "Removing ordering edges when not required\n");
	for (list<HyperOp*>::iterator vertexItr = Vertices.begin(); vertexItr != Vertices.end(); vertexItr++) {
			...
			for (map<HyperOpEdge*, HyperOp*>::iterator childEdgeItr = vertex->ChildMap.begin(); childEdgeItr != vertex->ChildMap.end(); childEdgeItr++) {
				if (childEdgeItr->second == childVertex) {
					...
					if (edge->getType() == HyperOpEdge::ORDERING) {
						...
						otherCommunicationEdgesExist = true;
					}
				}
			}

			if (otherCommunicationEdgesExist && !orderingEdges.empty()) {
...
//	DEBUG(dbgs() << "Removing ordering edges\n");
//	for (list<HyperOp*>::iterator vertexItr = Vertices.begin(); vertexItr != Vertices.end(); vertexItr++) {
//		HyperOp* vertex = *vertexItr;
//		list<HyperOp*> children = vertex->getChildList();
//		for (list<HyperOp*>::iterator childItr = children.begin(); childItr != children.end(); childItr++) {
...
//				if (immediateDominator == parentPredicate.second || pathExistsInHIG(immediateDominator, parentPredicate.second)) {
			...
			for (map<HyperOpEdge*, HyperOp*>::iterator parentItr = hyperOp->ParentMap.begin(); parentItr != hyperOp->ParentMap.end(); parentItr++) {
				if ((parentItr->first->getType() == HyperOpEdge::SCALAR || parentItr->first->getType() == HyperOpEdge::CONTEXT_FRAME_ADDRESS_SCALAR) && parentItr->second != immediateDominator && pathExistsInHIG(parentProducingPredicate, parentItr->second)) {
					decByValue++;
				}
			}
			if (decByValue > 0) {
	...
	DEBUG(dbgs() << "Decrementing sync count in case syncs originate from mutually exclusive paths to a HyperOp\n");
//Update the sync count of nodes with sync edges incoming from mutually exclusive paths
	for (list<HyperOp*>::iterator hopItr = this->Vertices.begin(); hopItr != this->Vertices.end(); hopItr++) {
		...
		if (hyperOp->isBarrierHyperOp()) {
			...
			//Find out if there are predicates complementary to each other
			...
			for (list<HyperOp*>::iterator syncSourceItr = syncSourceList.begin(); syncSourceItr != syncSourceList.end(); syncSourceItr++) {
				if (*syncSourceItr != syncSourceList.back()) {
					...
					for (; secondSyncSourceItr != syncSourceList.end(); secondSyncSourceItr++) {
						list<pair<HyperOpEdge*, HyperOp*> > firstParentPredicateChain = lastPredicateInput(controlFlowGraphAndOriginalHopMap.second[*syncSourceItr]);
						list<pair<HyperOpEdge*, HyperOp*> > secondParentPredicateChain = lastPredicateInput(controlFlowGraphAndOriginalHopMap.second[*secondSyncSourceItr]);
						HyperOpEdge* firstParentPredicate;
						HyperOpEdge* secondParentPredicate;
						...
						if (mutuallyExclusiveHyperOps(controlFlowGraphAndOriginalHopMap.second[*syncSourceItr], controlFlowGraphAndOriginalHopMap.second[*secondSyncSourceItr])) {
							//Sync decrement count is set to the maximum of sync sources on mutually exclusive paths
