...
namespace {
class REDEFINEObjectWriter : public MCELFObjectTargetWriter {
public:
  REDEFINEObjectWriter(uint8_t OSABI);

  virtual ~REDEFINEObjectWriter();
...
REDEFINEObjectWriter::REDEFINEObjectWriter(uint8_t OSABI)
  : MCELFObjectTargetWriter(/*Is64Bit=*/true, OSABI, ELF::EM_S390,
                            /*HasRelocationAddend=*/ true) {}

REDEFINEObjectWriter::~REDEFINEObjectWriter() {
}
...
static unsigned getPCRelReloc(unsigned Kind) {
  ...
  case REDEFINE::FK_390_PC16DBL:  return ELF::R_390_PC16DBL;
  case REDEFINE::FK_390_PC32DBL:  return ELF::R_390_PC32DBL;
  case REDEFINE::FK_390_PLT16DBL: return ELF::R_390_PLT16DBL;
  case REDEFINE::FK_390_PLT32DBL: return ELF::R_390_PLT32DBL;
  }
...
static unsigned getPLTReloc(unsigned Kind) {
  switch (Kind) {
  case REDEFINE::FK_390_PC16DBL: return ELF::R_390_PLT16DBL;
  case REDEFINE::FK_390_PC32DBL: return ELF::R_390_PLT32DBL;
  }
  llvm_unreachable("Unsupported absolute address");
}

unsigned REDEFINEObjectWriter::GetRelocType(const MCValue &Target,
                                           const MCFixup &Fixup,
                                           bool IsPCRel,
  ...
  case MCSymbolRefExpr::VK_GOT:
    if (IsPCRel && Kind == REDEFINE::FK_390_PC32DBL)
...
const MCSymbol *REDEFINEObjectWriter::ExplicitRelSym(const MCAssembler &Asm,
                                                    const MCValue &Target,
                                                    const MCFragment &F,
                                                    const MCFixup &Fixup,
                                                    bool IsPCRel) const {
...
MCObjectWriter *llvm::createREDEFINEObjectWriter(raw_ostream &OS,
                                                uint8_t OSABI) {
  MCELFObjectTargetWriter *MOTW = new REDEFINEObjectWriter(OSABI);
