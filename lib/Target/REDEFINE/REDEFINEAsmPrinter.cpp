	...
	for (int pHyperOpIndex = 0; pHyperOpIndex < pHyperOpInstructions.size();
		...
		for (list<const MachineInstr*>::iterator mcItr = pHyperOpItr.begin();
			...
			if (bb->getNumber() == 0 && *mcItr == bb->begin() && frameIndex == true) {
				...
				globalAddressCode.append("\tsrli	x3, x30, ").append(itostr(REDEFINEMCInstrScheduler::SHIFT_FOR_CRID)).append("\n");
				globalAddressCode.append("\tsrli	x4, x30, ").append(itostr(REDEFINEMCInstrScheduler::SHIFT_FOR_PAGENUMBER)).append("\n");
				globalAddressCode.append("\tandi	x5, x4, ").append(itostr(REDEFINEMCInstrScheduler::PAGE_NUMBER_MASK)).append("\n");
				globalAddressCode.append("\tsrli	x6, x30, ").append(itostr(REDEFINEMCInstrScheduler::SHIFT_FOR_FRAMENUMBER)).append("\n");
				globalAddressCode.append("\tandi	x7, x6, ").append(itostr(REDEFINEMCInstrScheduler::FRAME_NUMBER_MASK)).append("\n");
				globalAddressCode.append("\taddi	x8, x0, ").append(itostr(REDEFINEMCInstrScheduler::FRAMES_PER_CR)).append("\n");
				globalAddressCode.append("\tmul		x9, x3, x8\n");
				globalAddressCode.append("\taddi	x26, x0, ").append(itostr(REDEFINEMCInstrScheduler::FRAMES_PER_PAGE)).append("\n");
				globalAddressCode.append("\tmul		x27, x5, x26\n");
				globalAddressCode.append("\tadd		x28, x27, x7\n");
				globalAddressCode.append("\tadd		x29, x28, x9\n");
				globalAddressCode.append("\tmul		x31, x29, x2\n");
				globalAddressCode.append("\tadd		x30, x1, x31\n");
