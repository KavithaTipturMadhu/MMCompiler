//===-- REDEFINEISelLowering.h - REDEFINE DAG lowering interface --*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that REDEFINE uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_TARGET_REDEFINE_ISELLOWERING_H
#define LLVM_TARGET_REDEFINE_ISELLOWERING_H
...
class REDEFINETargetLowering: public TargetLowering {
public:
	explicit REDEFINETargetLowering(REDEFINETargetMachine &TM);
	...
	getRegForInlineAsmConstraint(const std::string &Constraint, EVT VT) const LLVM_OVERRIDE;
	...
	getSingleConstraintMatchWeight(AsmOperandInfo &info, const char *constraint) const LLVM_OVERRIDE;
	virtual void
	LowerAsmOperandForConstraint(SDValue Op, std::string &Constraint, std::vector<SDValue> &Ops, SelectionDAG &DAG) const LLVM_OVERRIDE;
	virtual MachineBasicBlock *
	EmitInstrWithCustomInserter(MachineInstr *MI, MachineBasicBlock *BB) const LLVM_OVERRIDE;
	virtual SDValue LowerOperation(SDValue Op, SelectionDAG &DAG) const LLVM_OVERRIDE;
	virtual SDValue
	LowerFormalArguments(SDValue Chain, CallingConv::ID CallConv, bool isVarArg, const SmallVectorImpl<ISD::InputArg> &Ins, DebugLoc DL, SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const LLVM_OVERRIDE;
	virtual SDValue
	LowerCall(CallLoweringInfo &CLI, SmallVectorImpl<SDValue> &InVals) const LLVM_OVERRIDE;

	virtual bool
	CanLowerReturn(CallingConv::ID CallConv, MachineFunction &MF, bool isVarArg, const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const;

	virtual SDValue
	LowerReturn(SDValue Chain, CallingConv::ID CallConv, bool IsVarArg, const SmallVectorImpl<ISD::OutputArg> &Outs, const SmallVectorImpl<SDValue> &OutVals, DebugLoc DL, SelectionDAG &DAG) const LLVM_OVERRIDE;
	...
	/// REDEFINECC - This class provides methods used to analyze formal and call
	/// arguments and inquire about calling convention information.
	class REDEFINECC {
	public:
		REDEFINECC(CallingConv::ID CallConv, bool IsREDEFINE32, CCState &Info, const REDEFINESubtarget &Subtarget);
	...
	private:
		...
		void allocateRegs(ByValArgInfo &ByVal, unsigned ByValSize, unsigned Align, MVT ValVT);
		...
		void analyzeReturn(const SmallVectorImpl<Ty> &RetVals, bool IsSoftFloat, const SDNode *CallNode, const Type *RetTy) const;
...
private:
	...
	SDValue lowerGlobalTLSAddress(GlobalAddressSDNode *Node, SelectionDAG &DAG) const;
	...
	// argument.
	void copyByValRegs(SDValue Chain, DebugLoc DL, std::vector<SDValue> &OutChains, SelectionDAG &DAG, const ISD::ArgFlagsTy &Flags, SmallVectorImpl<SDValue> &InVals, const Argument *FuncArg, const REDEFINECC &CC, const ByValArgInfo &ByVal) const;
	/// passByValArg - Pass a byval argument in registers or on stack.
	void passByValArg(SDValue Chain, DebugLoc DL, std::deque<std::pair<unsigned, SDValue> > &RegsToPass, SmallVector<SDValue, 8> &MemOpChains, SDValue StackPtr, MachineFrameInfo *MFI, SelectionDAG &DAG, SDValue Arg, const REDEFINECC &CC, const ByValArgInfo &ByVal, const ISD::ArgFlagsTy &Flags,
			bool isLittle) const;
	// writeVarArgRegs - Write variable function arguments passed in registers
	// to the stack. Also create a stack frame object for the first variable
	// argument.
	void writeVarArgRegs(std::vector<SDValue> &OutChains, const REDEFINECC &CC, SDValue Chain, DebugLoc DL, SelectionDAG &DAG) const;
};
} // end namespace llvm

#endif // LLVM_TARGET_REDEFINE_ISELLOWERING_H
