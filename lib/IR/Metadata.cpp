...
MDString *MDString::get(LLVMContext &Context, StringRef Str) {
	...
	StringMapEntry<Value*> &Entry = pImpl->MDStringCache.GetOrCreateValue(Str);
	Value *&S = Entry.getValue();
	if (!S)
		S = new MDString(Context);
...
MDNode::~MDNode() {
	assert(
			(getSubclassDataFromValue() & DestroyFlag) != 0
...
MDNode *MDNode::getWhenValsUnresolved(LLVMContext &Context,
		ArrayRef<Value*> Vals, bool isFunctionLocal) {
...
MDNode *MDNode::getTemporary(LLVMContext &Context, ArrayRef<Value*> Vals) {
	MDNode *N = (MDNode *) malloc(
			sizeof(MDNode) + Vals.size() * sizeof(MDNodeOperand));
...
void MDNode::deleteTemporary(MDNode *N) {
	...
	assert(
			!N->getContext().pImpl->NonUniquedMDNodes.erase(N)
					&& "Deleting a non-temporary non-uniqued node!");
	assert(
			(N->getSubclassDataFromValue() & NotUniquedBit)
					&& "Temporary MDNode does not have NotUniquedBit set!");
	assert(
			(N->getSubclassDataFromValue() & DestroyFlag) == 0
	...
	if (isFunctionLocal() && !isFunctionLocalValue(To)) {
		...
		if (!isStillFunctionLocal)
			setValueSubclassData(
					getSubclassDataFromValue() & ~FunctionLocalBit);
	}
}
...
NamedMDNode::NamedMDNode(const Twine &N) :
		Name(N.str()), Parent(0), Operands(
				new SmallVector<TrackingVH<MDNode>, 4>()) {
}
...
void Instruction::getAllMetadataImpl(
	...
	if (!DbgLoc.isUnknown()) {
		Result.push_back(
				std::make_pair((unsigned) LLVMContext::MD_dbg,
