...
void MachineOperand::substVirtReg(unsigned Reg, unsigned SubIdx, const TargetRegisterInfo &TRI) {
...
void MachineOperand::ChangeToRegister(unsigned Reg, bool isDef, bool isImp, bool isKill, bool isDead, bool isUndef, bool isDebug) {
...
bool MachineOperand::isIdenticalTo(const MachineOperand &Other) const {
	if (getType() != Other.getType() || getTargetFlags() != Other.getTargetFlags())
		return false;

	switch (getType()) {
	case MachineOperand::MO_Register:
		return getReg() == Other.getReg() && isDef() == Other.isDef() && getSubReg() == Other.getSubReg();
	...
	case MachineOperand::MO_ExternalSymbol:
		return !strcmp(getSymbolName(), Other.getSymbolName()) && getOffset() == Other.getOffset();
	case MachineOperand::MO_BlockAddress:
		return getBlockAddress() == Other.getBlockAddress() && getOffset() == Other.getOffset();
...
hash_code llvm::hash_value(const MachineOperand &MO) {
	...
	case MachineOperand::MO_ConstantPoolIndex:
	case MachineOperand::MO_TargetIndex:
		return hash_combine(MO.getType(), MO.getTargetFlags(), MO.getIndex(), MO.getOffset());
	case MachineOperand::MO_JumpTableIndex:
		return hash_combine(MO.getType(), MO.getTargetFlags(), MO.getIndex());
	case MachineOperand::MO_ExternalSymbol:
		return hash_combine(MO.getType(), MO.getTargetFlags(), MO.getOffset(), MO.getSymbolName());
	case MachineOperand::MO_GlobalAddress:
		return hash_combine(MO.getType(), MO.getTargetFlags(), MO.getGlobal(), MO.getOffset());
	case MachineOperand::MO_BlockAddress:
		return hash_combine(MO.getType(), MO.getTargetFlags(), MO.getBlockAddress(), MO.getOffset());
...
void MachineOperand::print(raw_ostream &OS, const TargetMachine *TM) const {
	...
	case MachineOperand::MO_Register:
		...
		if (isDef() || isKill() || isDead() || isImplicit() || isUndef() || isInternalRead() || isEarlyClobber() || isTied()) {
			OS << '<';
...
MachineMemOperand::MachineMemOperand(MachinePointerInfo ptrinfo, unsigned f, uint64_t s, unsigned int a, const MDNode *TBAAInfo, const MDNode *Ranges) :
		PtrInfo(ptrinfo), Size(s), Flags((f & ((1 << MOMaxBits) - 1)) | ((Log2_32(a) + 1) << MOMaxBits)), TBAAInfo(TBAAInfo), Ranges(Ranges) {
	assert((PtrInfo.V == 0 || isa<PointerType>(PtrInfo.V->getType())) && "invalid pointer value");
...
void MachineMemOperand::refineAlignment(const MachineMemOperand *MMO) {
	...
	if (MMO->getBaseAlignment() >= getBaseAlignment()) {
		...
		Flags = (Flags & ((1 << MOMaxBits) - 1)) | ((Log2_32(MMO->getBaseAlignment()) + 1) << MOMaxBits);
...
raw_ostream &llvm::operator<<(raw_ostream &OS, const MachineMemOperand &MMO) {
	...
	if (MMO.getBaseAlignment() != MMO.getAlignment() || MMO.getBaseAlignment() != MMO.getSize())
...
MachineInstr::MachineInstr(MachineFunction &MF, const MCInstrDesc &tid, const DebugLoc dl, bool NoImp) :
		MCID(&tid), Parent(0), Operands(0), NumOperands(0), Flags(0), AsmPrinterFlags(0), NumMemRefs(0), MemRefs(0), debugLoc(dl) {
...
MachineInstr::MachineInstr(MachineFunction &MF, const MachineInstr &MI) :
		MCID(&MI.getDesc()), Parent(0), Operands(0), NumOperands(0), Flags(0), AsmPrinterFlags(0), NumMemRefs(MI.NumMemRefs), MemRefs(MI.MemRefs), debugLoc(MI.getDebugLoc()) {
...
void MachineInstr::addOperand(MachineFunction &MF, const MachineOperand &Op) {
	...
	assert((isImpReg || Op.isRegMask() || MCID->isVariadic() || OpNo < MCID->getNumOperands()) && "Trying to add an operand to a machine instr that is already done!");
...
bool MachineInstr::isIdenticalTo(const MachineInstr *Other, MICheckType Check) const {
	...
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		...
		if (MO.isDef()) {
			if (Check == IgnoreDefs)
				...
				if (TargetRegisterInfo::isPhysicalRegister(MO.getReg()) || TargetRegisterInfo::isPhysicalRegister(OMO.getReg()))
	...
	if (isDebugValue())
		if (!getDebugLoc().isUnknown() && !Other->getDebugLoc().isUnknown() && getDebugLoc() != Other->getDebugLoc())
			return false;
	return true;
}
...
int MachineInstr::findInlineAsmFlagIdx(unsigned OpIdx, unsigned *GroupNo) const {
	...
	unsigned Group = 0;
	unsigned NumOps;
	for (unsigned i = InlineAsm::MIOp_FirstOperand, e = getNumOperands(); i < e; i += NumOps) {
...
MachineInstr::getRegClassConstraint(unsigned OpIdx, const TargetInstrInfo *TII, const TargetRegisterInfo *TRI) const {
...
int MachineInstr::findRegisterUseOperandIdx(unsigned Reg, bool isKill, const TargetRegisterInfo *TRI) const {
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		const MachineOperand &MO = getOperand(i);
		...
		if (!MOReg)
			continue;
		if (MOReg == Reg || (TRI && TargetRegisterInfo::isPhysicalRegister(MOReg) && TargetRegisterInfo::isPhysicalRegister(Reg) && TRI->isSubRegister(MOReg, Reg)))
			if (!isKill || MO.isKill())
				return i;
	}
	return -1;
...
/// partial defines.
std::pair<bool, bool> MachineInstr::readsWritesVirtualRegister(unsigned Reg, SmallVectorImpl<unsigned> *Ops) const {
...
int MachineInstr::findRegisterDefOperandIdx(unsigned Reg, bool isDead, bool Overlap, const TargetRegisterInfo *TRI) const {
	...
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		...
		if (!Found && TRI && isPhys && TargetRegisterInfo::isPhysicalRegister(MOReg)) {
			if (Overlap)
...
void MachineInstr::substituteRegister(unsigned FromReg, unsigned ToReg, unsigned SubIdx, const TargetRegisterInfo &RegInfo) {
...
bool MachineInstr::isSafeToMove(const TargetInstrInfo *TII, AliasAnalysis *AA, bool &SawStore) const {
	// Ignore stuff that we obviously can't move.
	//
	// Treat volatile loads as stores. This is not strictly necessary for
	// volatiles, but it is required for atomic loads. It is not allowed to move
	// a load across an atomic load with Ordering > Monotonic.
	if (mayStore() || isCall() || (mayLoad() && hasOrderedMemoryRef())) {
		SawStore = true;
		return false;
	}

	if (isLabel() || isDebugValue() || isTerminator() || hasUnmodeledSideEffects())
		return false;
...
bool MachineInstr::isSafeToReMat(const TargetInstrInfo *TII, AliasAnalysis *AA, unsigned DstReg) const {
	bool SawStore = false;
	if (!TII->isTriviallyReMaterializable(this, AA) || !isSafeToMove(TII, AA, SawStore))
		return false;
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		const MachineOperand &MO = getOperand(i);
		if (!MO.isReg())
...
bool MachineInstr::isInvariantLoad(AliasAnalysis *AA) const {
	...
	for (mmo_iterator I = memoperands_begin(), E = memoperands_end(); I != E; ++I) {
		...
		if (const Value *V = (*I)->getValue()) {
			...
			if (AA && AA->pointsToConstantMemory(AliasAnalysis::Location(V, (*I)->getSize(), (*I)->getTBAAInfo())))
				continue;
		}
...
unsigned MachineInstr::isConstantValuePHI() const {
	if (!isPHI())
		return 0;
	assert(getNumOperands() >= 3 && "It's illegal to have a PHI without source operands");
...
void MachineInstr::copyImplicitOps(MachineFunction &MF, const MachineInstr *MI) {
	for (unsigned i = MI->getDesc().getNumOperands(), e = MI->getNumOperands(); i != e; ++i) {
...
void MachineInstr::print(raw_ostream &OS, const TargetMachine *TM, bool SkipOpers) const {
	...
	for (; StartOp < e && getOperand(StartOp).isReg() && getOperand(StartOp).isDef() && !getOperand(StartOp).isImplicit(); ++StartOp) {
		if (StartOp != 0)
			OS << ", ";
	...
	for (unsigned i = StartOp, e = getNumOperands(); i != e; ++i) {
		...
		if (MF && isCall() && MO.isReg() && MO.isImplicit() && MO.isDef()) {
			unsigned Reg = MO.getReg();
			if (TargetRegisterInfo::isPhysicalRegister(Reg)) {
				const MachineRegisterInfo &MRI = MF->getRegInfo();
				if (MRI.use_empty(Reg)) {
					bool HasAliasLive = false;
					for (MCRegAliasIterator AI(Reg, TM->getRegisterInfo(), true); AI.isValid(); ++AI) {
...
bool MachineInstr::addRegisterKilled(unsigned IncomingReg, const TargetRegisterInfo *RegInfo, bool AddIfNotFound) {
	bool isPhysReg = TargetRegisterInfo::isPhysicalRegister(IncomingReg);
	bool hasAliases = isPhysReg && MCRegAliasIterator(IncomingReg, RegInfo, false).isValid();
	bool Found = false;
	...
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		...
		} else if (hasAliases && MO.isKill() && TargetRegisterInfo::isPhysicalRegister(Reg)) {
	...
	if (!Found && AddIfNotFound) {
		addOperand(MachineOperand::CreateReg(IncomingReg, false /*IsDef*/, true /*IsImp*/, true /*IsKill*/));
		return true;
	}
	return Found;
}

void MachineInstr::clearRegisterKills(unsigned Reg, const TargetRegisterInfo *RegInfo) {
	if (!TargetRegisterInfo::isPhysicalRegister(Reg))
		RegInfo = 0;
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		MachineOperand &MO = getOperand(i);
...
bool MachineInstr::addRegisterDead(unsigned IncomingReg, const TargetRegisterInfo *RegInfo, bool AddIfNotFound) {
	bool isPhysReg = TargetRegisterInfo::isPhysicalRegister(IncomingReg);
	bool hasAliases = isPhysReg && MCRegAliasIterator(IncomingReg, RegInfo, false).isValid();
	bool Found = false;
	SmallVector<unsigned, 4> DeadOps;
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		...
		if (Reg == IncomingReg) {
			MO.setIsDead();
			Found = true;
		} else if (hasAliases && MO.isDead() && TargetRegisterInfo::isPhysicalRegister(Reg)) {
	...
	addOperand(MachineOperand::CreateReg(IncomingReg, true /*IsDef*/, true /*IsImp*/, false /*IsKill*/, true /*IsDead*/));
	return true;
}

void MachineInstr::addRegisterDefined(unsigned IncomingReg, const TargetRegisterInfo *RegInfo) {
	if (TargetRegisterInfo::isPhysicalRegister(IncomingReg)) {
		...
		for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
			const MachineOperand &MO = getOperand(i);
			if (MO.isReg() && MO.getReg() == IncomingReg && MO.isDef() && MO.getSubReg() == 0)
				return;
		}
	}
	addOperand(MachineOperand::CreateReg(IncomingReg, true /*IsDef*/, true /*IsImp*/));
}

void MachineInstr::setPhysRegsDeadExcept(ArrayRef<unsigned> UsedRegs, const TargetRegisterInfo &TRI) {
	...
	for (unsigned i = 0, e = getNumOperands(); i != e; ++i) {
		...
		if (!TargetRegisterInfo::isPhysicalRegister(Reg))
			continue;
		bool Dead = true;
		for (ArrayRef<unsigned>::iterator I = UsedRegs.begin(), E = UsedRegs.end(); I != E; ++I)
...
void MachineInstr::emitError(StringRef Msg) const {
	...
	for (unsigned i = getNumOperands(); i != 0; --i) {
		if (getOperand(i - 1).isMetadata() && (LocMD = getOperand(i - 1).getMetadata()) && LocMD->getNumOperands() != 0) {
