...
MachineInstr *LiveVariables::FindLastPartialDef(unsigned Reg, SmallSet<unsigned, 4> &PartDefRegs) {
...
void LiveVariables::HandlePhysRegUse(unsigned Reg, MachineInstr *MI) {
	...
	if (!LastDef && !PhysRegUse[Reg]) {
		...
		if (LastPartialDef) {
			LastPartialDef->addOperand(MachineOperand::CreateReg(Reg, true/*IsDef*/, true/*IsImp*/));
			...
			for (MCSubRegIterator SubRegs(Reg, TRI); SubRegs.isValid(); ++SubRegs) {
				...
				// here.
				LastPartialDef->addOperand(MachineOperand::CreateReg(SubReg, false/*IsDef*/, true/*IsImp*/));
		...
		LastDef->addOperand(MachineOperand::CreateReg(Reg, true/*IsDef*/, true/*IsImp*/));
...
bool LiveVariables::HandlePhysRegKill(unsigned Reg, MachineInstr *MI) {
	...
	if (!PhysRegUse[Reg]) {
		...
		for (MCSubRegIterator SubRegs(Reg, TRI); SubRegs.isValid(); ++SubRegs) {
			...
			if (NeedDef)
				PhysRegDef[Reg]->addOperand(MachineOperand::CreateReg(SubReg, true/*IsDef*/, true/*IsImp*/));
		...
		if (LastPartDef)
			...
			LastPartDef->addOperand(MachineOperand::CreateReg(Reg, false/*IsDef*/, true/*IsImp*/, true/*IsKill*/));
		else {
			MachineOperand *MO = LastRefOrPartRef->findRegisterDefOperand(Reg, false, TRI);
...
void LiveVariables::HandlePhysRegDef(unsigned Reg, MachineInstr *MI, SmallVector<unsigned, 4> &Defs) {
...
void LiveVariables::UpdatePhysRegDefs(MachineInstr *MI, SmallVector<unsigned, 4> &Defs) {
	while (!Defs.empty()) {
...
bool LiveVariables::runOnMachineFunction(MachineFunction &mf) {
	...
	for (df_ext_iterator<MachineBasicBlock*, SmallPtrSet<MachineBasicBlock*, 16> > DFI = df_ext_begin(Entry, Visited), E = df_ext_end(Entry, Visited); DFI != E; ++DFI) {
		MachineBasicBlock *MBB = *DFI;

		// Mark live-in registers as live-in.
		SmallVector<unsigned, 4> Defs;
		for (MachineBasicBlock::livein_iterator II = MBB->livein_begin(), EE = MBB->livein_end(); II != EE; ++II) {
			assert(TargetRegisterInfo::isPhysicalRegister(*II) && "Cannot have a live-in virtual register!");
		...
		if (!PHIVarInfo[MBB->getNumber()].empty()) {
			...
			for (SmallVector<unsigned, 4>::iterator I = VarInfoVec.begin(), E = VarInfoVec.end(); I != E; ++I)
				// Mark it alive only in the block we are representing.
				MarkVirtRegAliveInBlock(getVarInfo(*I), MRI->getVRegDef(*I)->getParent(), MBB);
		}

		// MachineCSE may CSE instructions which write to non-allocatable physical
		// registers across MBBs. Remember if any reserved register is liveout.
		SmallSet<unsigned, 4> LiveOuts;
		for (MachineBasicBlock::const_succ_iterator SI = MBB->succ_begin(), SE = MBB->succ_end(); SI != SE; ++SI) {
			MachineBasicBlock *SuccMBB = *SI;
			if (SuccMBB->isLandingPad())
				continue;
			for (MachineBasicBlock::livein_iterator LI = SuccMBB->livein_begin(), LE = SuccMBB->livein_end(); LI != LE; ++LI) {
				unsigned LReg = *LI;
...
void LiveVariables::replaceKillInstruction(unsigned Reg, MachineInstr *OldMI, MachineInstr *NewMI) {
...
void LiveVariables::analyzePHINodes(const MachineFunction& Fn) {
	for (MachineFunction::const_iterator I = Fn.begin(), E = Fn.end(); I != E; ++I)
		for (MachineBasicBlock::const_iterator BBI = I->begin(), BBE = I->end(); BBI != BBE && BBI->isPHI(); ++BBI)
			for (unsigned i = 1, e = BBI->getNumOperands(); i != e; i += 2)
				if (BBI->getOperand(i).readsReg())
					PHIVarInfo[BBI->getOperand(i + 1).getMBB()->getNumber()].push_back(BBI->getOperand(i).getReg());
}

bool LiveVariables::VarInfo::isLiveIn(const MachineBasicBlock &MBB, unsigned Reg, MachineRegisterInfo &MRI) {
...
/// BB.
void LiveVariables::addNewBlock(MachineBasicBlock *BB, MachineBasicBlock *DomBB, MachineBasicBlock *SuccBB) {
	...
	for (; BBI != BBE; ++BBI) {
		for (MachineInstr::mop_iterator I = BBI->operands_begin(), E = BBI->operands_end(); I != E; ++I) {
