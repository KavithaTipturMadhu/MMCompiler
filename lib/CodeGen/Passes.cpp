...
static cl::opt<cl::boolOrDefault> OptimizeRegAlloc("optimize-regalloc",
		cl::Hidden,
		cl::desc("Enable optimized register allocation compilation path."));
static cl::opt<cl::boolOrDefault> EnableMachineSched("enable-misched",
		cl::Hidden,
		cl::desc("Enable the machine instruction scheduling pass."));
static cl::opt<bool> EnableStrongPHIElim("strong-phi-elim", cl::Hidden,
		cl::desc("Use strong PHI elimination."));
static cl::opt<bool> DisablePostRAMachineLICM("disable-postra-machine-licm",
		cl::Hidden, cl::desc("Disable Machine LICM"));
...
static cl::opt<std::string> PrintMachineInstrs("print-machineinstrs",
		cl::ValueOptional, cl::desc("Print machine instrs"),
		cl::value_desc("pass-name"), cl::init("option-unspecified"));
//REDEFINE
static cl::opt<std::string> PrintREDEFINEHIG("print-redefine-hig",
		cl::ValueOptional, cl::desc("Print REDEFINE HIG"),
		cl::value_desc("pass-name"), cl::init("option-unspecified"));
...
static IdentifyingPassPtr applyOverride(IdentifyingPassPtr TargetID,
		cl::boolOrDefault Override, AnalysisID StandardID) {
	switch (Override) {
	case cl::BOU_UNSET:
		return TargetID;
	case cl::BOU_TRUE:
...
TargetPassConfig::TargetPassConfig(TargetMachine *tm, PassManagerBase &pm) :
		ImmutablePass(ID), PM(&pm), StartAfter(0), StopAfter(0), Started(true), Stopped(
				false), TM(tm), Impl(0), Initialized(false), DisableVerify(
				false), EnableTailMerge(true) {
...
AnalysisID TargetPassConfig::addPass(AnalysisID PassID) {
	...
	for (SmallVector<std::pair<AnalysisID, IdentifyingPassPtr>, 4>::iterator I =
			Impl->InsertedPasses.begin(), E = Impl->InsertedPasses.end();
			I != E; ++I) {
		if ((*I).first == PassID) {
...
void TargetPassConfig::addMachinePasses() {
	...
	else if (!StringRef(PrintMachineInstrs.getValue()).equals(
			"option-unspecified")) {
...
static RegisterRegAlloc defaultRegAlloc("default",
		"pick register allocator based on -O option",
		useDefaultRegisterAllocator);

/// -regalloc=... command line option.
static cl::opt<RegisterRegAlloc::FunctionPassCtor, false,
		RegisterPassParser<RegisterRegAlloc> > RegAlloc("regalloc",
