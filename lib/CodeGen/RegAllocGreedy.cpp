...
static cl::opt<SplitEditor::ComplementSpillMode> SplitSpillMode("split-spill-mode", cl::Hidden, cl::desc("Spill mode for splitting live ranges"),
		...
		clEnumValEnd), cl::init(SplitEditor::SM_Partition));

static RegisterRegAlloc greedyRegAlloc("greedy", "greedy register allocator", createGreedyRegisterAllocator);

namespace {
class RAGreedy: public MachineFunctionPass, public RegAllocBase, private LiveRangeEdit::Delegate {
	//TODO REDEFINE specific requirement, need to move this to RegAllocBase or some other extension of the class later
	list<unsigned> liveInPhysicalRegs;
	list<unsigned> liveInVirtualRegs;
	...
	struct EvictionCost {
		...
		EvictionCost(unsigned B = 0) :
				BrokenHints(B), MaxWeight(0) {
		}
...
public:
	...
	virtual unsigned selectOrSplit(LiveInterval&, SmallVectorImpl<LiveInterval*>&);
...
private:
	...
	void evictInterference(LiveInterval&, unsigned, SmallVectorImpl<LiveInterval*>&);

	unsigned tryAssign(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&);
	unsigned tryEvict(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&, unsigned = ~0u);
	unsigned tryRegionSplit(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&);
	unsigned tryBlockSplit(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&);
	unsigned tryInstructionSplit(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&);
	unsigned tryLocalSplit(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&);
	unsigned trySplit(LiveInterval&, AllocationOrder&, SmallVectorImpl<LiveInterval*>&);
};
...
#ifndef NDEBUG
const char * const RAGreedy::StageName[] = { "RS_New", "RS_Assign", "RS_Split", "RS_Split2", "RS_Spill", "RS_Done" };
#endif
...
void RAGreedy::enqueue(LiveInterval *LI) {
	...
	assert(TargetRegisterInfo::isVirtualRegister(Reg) && "Can only enqueue virtual registers");
	unsigned Prio;
...
bool RAGreedy::shouldEvict(LiveInterval &A, bool IsHint, LiveInterval &B, bool BreaksHint) {
...
bool RAGreedy::canEvictInterference(LiveInterval &VirtReg, unsigned PhysReg, bool IsHint, EvictionCost &MaxCost) {
	...
	for (MCRegUnitIterator Units(PhysReg, TRI); Units.isValid(); ++Units) {
		...
		for (unsigned i = Q.interferingVRegs().size(); i; --i) {
			...
			assert(TargetRegisterInfo::isVirtualRegister(Intf->reg) && "Only expecting virtual register interference from query");
			...
			bool Urgent = !VirtReg.isSpillable() && (Intf->isSpillable() || RegClassInfo.getNumAllocatableRegs(MRI->getRegClass(VirtReg.reg)) < RegClassInfo.getNumAllocatableRegs(MRI->getRegClass(Intf->reg)));
...
/// returned true.
void RAGreedy::evictInterference(LiveInterval &VirtReg, unsigned PhysReg, SmallVectorImpl<LiveInterval*> &NewVRegs) {
	...
	DEBUG(dbgs() << "evicting " << PrintReg(PhysReg, TRI) << " interference: Cascade " << Cascade << '\n');
	...
	for (unsigned i = 0, e = Intfs.size(); i != e; ++i) {
		...
		assert((ExtraRegInfo[Intf->reg].Cascade < Cascade || VirtReg.isSpillable() < Intf->isSpillable()) && "Cannot decrease cascade number, illegal eviction");
...
unsigned RAGreedy::tryEvict(LiveInterval &VirtReg, AllocationOrder &Order, SmallVectorImpl<LiveInterval*> &NewVRegs, unsigned CostPerUseLimit) {
	...
	if (CostPerUseLimit < ~0u) {
		...
		if (MinCost >= CostPerUseLimit) {
			DEBUG(dbgs() << RC->getName() << " minimum cost = " << MinCost << ", no cheaper registers to be found.\n");
			return 0;
		}
		...
		if (CostPerUseLimit == 1)
			if (unsigned CSR = RegClassInfo.getLastCalleeSavedAlias(PhysReg))
				if (!MRI->isPhysRegUsed(CSR)) {
					DEBUG(dbgs() << PrintReg(PhysReg, TRI) << " would clobber CSR " << PrintReg(CSR, TRI) << '\n');
					continue;
				}
...
bool RAGreedy::addSplitConstraints(InterferenceCache::Cursor Intf, float &Cost) {
...
void RAGreedy::addThroughConstraints(InterferenceCache::Cursor Intf, ArrayRef<unsigned> Blocks) {
...
void RAGreedy::growRegion(GlobalSplitCandidate &Cand) {
	...
	for (;;) {
		...
		for (int i = 0, e = NewBundles.size(); i != e; ++i) {
			...
			for (ArrayRef<unsigned>::iterator I = Blocks.begin(), E = Blocks.end(); I != E; ++I) {
				unsigned Block = *I;
				if (!Todo.test(Block))
					continue;
				Todo.reset(Block);
...
bool RAGreedy::calcCompactRegion(GlobalSplitCandidate &Cand) {
	...
	DEBUG( {
		for (int i = Cand.LiveBundles.find_first(); i >= 0; i = Cand.LiveBundles.find_next(i))
			dbgs() << " EB#" << i
			;
		dbgs() << ".\n"
		;
...
void RAGreedy::splitAroundRegion(LiveRangeEdit &LREdit, ArrayRef<unsigned> UsedCands) {
	...
	for (unsigned i = 0, e = LREdit.size(); i != e; ++i) {
		...
		if (IntvMap[i] < NumGlobalIntvs) {
			if (SA->countLiveBlocks(&Reg) >= OrigBlocks) {
				DEBUG(dbgs() << "Main interval covers the same " << OrigBlocks << " blocks as original.\n");
...
unsigned RAGreedy::tryRegionSplit(LiveInterval &VirtReg, AllocationOrder &Order, SmallVectorImpl<LiveInterval*> &NewVRegs) {
	unsigned NumCands = 0;
	...
	if (BestCand != NoCand) {
		...
		if (unsigned B = Cand.getBundles(BundleCand, BestCand)) {
			...
			DEBUG(dbgs() << "Split for " << PrintReg(Cand.PhysReg, TRI) << " in " << B << " bundles, intv " << Cand.IntvIdx << ".\n");
			(void) B;
		}
	}
	...
	if (HasCompact) {
		...
		if (unsigned B = Cand.getBundles(BundleCand, 0)) {
			...
			Cand.IntvIdx = SE->openIntv();
			DEBUG(dbgs() << "Split for compact region in " << B << " bundles, intv " << Cand.IntvIdx << ".\n");
			(void) B;
		}
	}
...
/// they don't allocate.
unsigned RAGreedy::tryBlockSplit(LiveInterval &VirtReg, AllocationOrder &Order, SmallVectorImpl<LiveInterval*> &NewVRegs) {
...
unsigned RAGreedy::tryInstructionSplit(LiveInterval &VirtReg, AllocationOrder &Order, SmallVectorImpl<LiveInterval*> &NewVRegs) {
...
///
void RAGreedy::calcGapWeights(unsigned PhysReg, SmallVectorImpl<float> &GapWeight) {
	...
	SlotIndex StopIdx = BI.LiveOut ? BI.LastInstr.getBoundaryIndex() : BI.LastInstr;

	GapWeight.assign(NumGaps, 0.0f);

	// Add interference from each overlapping register.
	for (MCRegUnitIterator Units(PhysReg, TRI); Units.isValid(); ++Units) {
		if (!Matrix->query(const_cast<LiveInterval&>(SA->getParent()), *Units).checkInterference())
			continue;
		...
		//
		LiveIntervalUnion::SegmentIter IntI = Matrix->getLiveUnions()[*Units].find(StartIdx);
...
/// basic block.
///
unsigned RAGreedy::tryLocalSplit(LiveInterval &VirtReg, AllocationOrder &Order, SmallVectorImpl<LiveInterval*> &NewVRegs) {
	assert(SA->getUseBlocks().size() == 1 && "Not a local interval");
	const SplitAnalysis::BlockInfo &BI = SA->getUseBlocks().front();
	...
	if (Matrix->checkRegMaskInterference(VirtReg)) {
		...
		unsigned ri = std::lower_bound(RMS.begin(), RMS.end(), Uses.front().getRegSlot()) - RMS.begin();
	...
	while (unsigned PhysReg = Order.next()) {
		...
		for (;;) {
			...
			DEBUG(dbgs() << PrintReg(PhysReg, TRI) << ' ' << Uses[SplitBefore] << '-' << Uses[SplitAfter] << " i=" << MaxGap);
			...
			if (Legal && MaxGap < HUGE_VALF) {
				...
				const float EstWeight = normalizeSpillWeight(blockFreq * (NewGaps + 1), Uses[SplitBefore].distance(Uses[SplitAfter]) + (LiveBefore + LiveAfter) * SlotIndex::InstrDist);
	...
	DEBUG(dbgs() << "Best local split range: " << Uses[BestBefore] << '-' << Uses[BestAfter] << ", " << BestDiff << ", " << (BestAfter - BestBefore + 1) << " instrs\n");
...
unsigned RAGreedy::trySplit(LiveInterval &VirtReg, AllocationOrder &Order, SmallVectorImpl<LiveInterval*>&NewVRegs) {
...
unsigned RAGreedy::selectOrSplit(LiveInterval &VirtReg, SmallVectorImpl<LiveInterval*> &NewVRegs) {
...
bool RAGreedy::runOnMachineFunction(MachineFunction &mf) {
	...
	RegAllocBase::init(getAnalysis<VirtRegMap>(), getAnalysis<LiveIntervals>(), getAnalysis<LiveRegMatrix>());
...
//		errs()<<PrintReg(liveInItr->second)<<",";
