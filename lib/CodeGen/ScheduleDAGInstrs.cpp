...
ScheduleDAGInstrs::ScheduleDAGInstrs(MachineFunction &mf, const MachineLoopInfo &mli, const MachineDominatorTree &mdt, bool IsPostRAFlag, LiveIntervals *lis) :
		ScheduleDAG(mf), MLI(mli), MDT(mdt), MFI(mf.getFrameInfo()), LIS(lis), IsPostRA(IsPostRAFlag), CanHandleTerminators(false), FirstDbgValue(0) {
	assert((IsPostRA || LIS) && "PreRA scheduling requires LiveIntervals");
	DbgValues.clear();
	assert(!(IsPostRA && MRI.getNumVirtRegs()) && "Virtual registers must be removed prior to PostRA scheduling");
...
static const Value *getUnderlyingObjectFromInt(const Value *V) {
	do {
		if (const Operator *U = dyn_cast<Operator>(V)) {
			...
			// identifiable object.
			if (U->getOpcode() != Instruction::Add || (!isa<ConstantInt>(U->getOperand(1)) && Operator::getOpcode(U->getOperand(1)) != Instruction::Mul && !isa<PHINode>(U->getOperand(1))))
				return V;
			V = U->getOperand(0);
		} else {
			return V;
...
static void getUnderlyingObjects(const Value *V, SmallVectorImpl<Value *> &Objects) {
	...
	do {
		...
		for (SmallVector<Value *, 4>::iterator I = Objs.begin(), IE = Objs.end(); I != IE; ++I) {
			V = *I;
			if (!Visited.insert(V))
				continue;
...
static void getUnderlyingObjectsForInstr(const MachineInstr *MI, const MachineFrameInfo *MFI, SmallVectorImpl<std::pair<const Value *, bool> > &Objects) {
	if (!MI->hasOneMemOperand() || !(*MI->memoperands_begin())->getValue() || (*MI->memoperands_begin())->isVolatile())
		return;
...
/// region.
void ScheduleDAGInstrs::enterRegion(MachineBasicBlock *bb, MachineBasicBlock::iterator begin, MachineBasicBlock::iterator end, unsigned endcount) {
...
void ScheduleDAGInstrs::addSchedBarrierDeps() {
	...
	bool AllDepKnown = ExitMI && (ExitMI->isCall() || ExitMI->isBarrier());
	if (ExitMI && AllDepKnown) {
		...
		for (MachineBasicBlock::succ_iterator SI = BB->succ_begin(), SE = BB->succ_end(); SI != SE; ++SI)
			for (MachineBasicBlock::livein_iterator I = (*SI)->livein_begin(), E = (*SI)->livein_end(); I != E; ++I) {
				unsigned Reg = *I;
...
void ScheduleDAGInstrs::addPhysRegDataDeps(SUnit *SU, unsigned OperIdx) {
	...
	for (MCRegAliasIterator Alias(MO.getReg(), TRI, true); Alias.isValid(); ++Alias) {
		...
		for (Reg2SUnitsMap::iterator I = Uses.find(*Alias); I != Uses.end(); ++I) {
			...
			if (UseOp < 0)
				...
				Dep.setMinLatency(SchedModel.computeOperandLatency(SU->getInstr(), OperIdx, RegUse, UseOp, /*FindMin=*/true));
			}
			Dep.setLatency(SchedModel.computeOperandLatency(SU->getInstr(), OperIdx, RegUse, UseOp, /*FindMin=*/false));
...
void ScheduleDAGInstrs::addPhysRegDeps(SUnit *SU, unsigned OperIdx) {
	...
	for (MCRegAliasIterator Alias(MO.getReg(), TRI, true); Alias.isValid(); ++Alias) {
		...
		for (Reg2SUnitsMap::iterator I = Defs.find(*Alias); I != Defs.end(); ++I) {
			...
			if (DefSU == &ExitSU)
				continue;
			if (DefSU != SU && (Kind != SDep::Output || !MO.isDead() || !DefSU->getInstr()->registerDefIsDead(*Alias))) {
				if (Kind == SDep::Anti)
					DefSU->addPred(SDep(SU, Kind, /*Reg=*/*Alias));
				else {
					SDep Dep(SU, Kind, /*Reg=*/*Alias);
					unsigned OutLatency = SchedModel.computeOutputLatency(MI, OperIdx, DefSU->getInstr());
...
static inline bool isGlobalMemoryObject(AliasAnalysis *AA, MachineInstr *MI) {
	if (MI->isCall() || MI->hasUnmodeledSideEffects() || (MI->hasOrderedMemoryRef() && (!MI->mayLoad() || !MI->isInvariantLoad(AA))))
		return true;
	return false;
}
...
static inline bool isUnsafeMemoryObject(MachineInstr *MI, const MachineFrameInfo *MFI) {
...
static bool MIsNeedChainEdge(AliasAnalysis *AA, const MachineFrameInfo *MFI, MachineInstr *MIa, MachineInstr *MIb) {
	...
	AliasAnalysis::AliasResult AAResult = AA->alias(AliasAnalysis::Location(MMOa->getValue(), Overlapa, MMOa->getTBAAInfo()), AliasAnalysis::Location(MMOb->getValue(), Overlapb, MMOb->getTBAAInfo()));
...
static unsigned iterateChainSucc(AliasAnalysis *AA, const MachineFrameInfo *MFI, SUnit *SUa, SUnit *SUb, SUnit *ExitSU, unsigned *Depth, SmallPtrSet<const SUnit*, 16> &Visited) {
	...
	if (SUa->isSucc(SUb) || isGlobalMemoryObject(AA, SUb->getInstr()))
		return *Depth;

	// If we do need an edge, or we have exceeded depth budget,
	// add that edge to the predecessors chain of SUb,
	// and stop descending.
	if (*Depth > 200 || MIsNeedChainEdge(AA, MFI, SUa->getInstr(), SUb->getInstr())) {
	...
	for (SUnit::const_succ_iterator I = SUb->Succs.begin(), E = SUb->Succs.end(); I != E; ++I)
		if (I->isCtrl())
...
static void adjustChainDeps(AliasAnalysis *AA, const MachineFrameInfo *MFI, SUnit *SU, SUnit *ExitSU, std::set<SUnit *> &CheckList, unsigned LatencyToLoad) {
	...
	for (std::set<SUnit *>::iterator I = CheckList.begin(), IE = CheckList.end(); I != IE; ++I) {
		if (SU == *I)
			continue;
		...
		for (SUnit::const_succ_iterator J = (*I)->Succs.begin(), JE = (*I)->Succs.end(); J != JE; ++J)
			if (J->isCtrl())
				iterateChainSucc(AA, MFI, SU, J->getSUnit(), ExitSU, &Depth, Visited);
	}
}
...
static inline
void addChainDependency(AliasAnalysis *AA, const MachineFrameInfo *MFI, SUnit *SUa, SUnit *SUb, std::set<SUnit *> &RejectList, unsigned TrueMemOrderLatency = 0, bool isNormalMemory = false) {
	// If this is a false dependency,
	// do not add the edge, but rememeber the rejected node.
	if (!EnableAASchedMI || MIsNeedChainEdge(AA, MFI, SUa->getInstr(), SUb->getInstr())) {
		...
		DEBUG(dbgs() << "\tReject chain dep between SU(" << SUa->NodeNum << ") and SU(" << SUb->NodeNum << ")\n");
	}
}
...
/// operands.
void ScheduleDAGInstrs::buildSchedGraph(AliasAnalysis *AA, RegPressureTracker *RPTracker) {
	...
	assert(Defs.empty() && Uses.empty() && "Only BuildGraph should update Defs/Uses");
	...
	for (MachineBasicBlock::iterator MII = RegionEnd, MIE = RegionBegin; MII != MIE; --MII) {
		...
		assert((CanHandleTerminators || (!MI->isTerminator() && !MI->isLabel())) && "Cannot schedule terminators or labels!");
		...
		if (isGlobalMemoryObject(AA, MI)) {
			...
			for (MapVector<const Value *, SUnit *>::iterator I = NonAliasMemDefs.begin(), E = NonAliasMemDefs.end(); I != E; ++I) {
				I->second->addPred(SDep(SU, SDep::Barrier));
			}
			for (MapVector<const Value *, std::vector<SUnit *> >::iterator I = NonAliasMemUses.begin(), E = NonAliasMemUses.end(); I != E; ++I) {
			...
			adjustChainDeps(AA, MFI, SU, &ExitSU, RejectMemNodes, TrueMemOrderLatency);
			...
			if (AliasChain) {
				...
				addChainDependency(AA, MFI, SU, AliasChain, RejectMemNodes, ChainLatency);
			}
			AliasChain = SU;
			for (unsigned k = 0, m = PendingLoads.size(); k != m; ++k)
				addChainDependency(AA, MFI, SU, PendingLoads[k], RejectMemNodes, TrueMemOrderLatency);
			for (MapVector<const Value *, SUnit *>::iterator I = AliasMemDefs.begin(), E = AliasMemDefs.end(); I != E; ++I)
				addChainDependency(AA, MFI, SU, I->second, RejectMemNodes);
			for (MapVector<const Value *, std::vector<SUnit *> >::iterator I = AliasMemUses.begin(), E = AliasMemUses.end(); I != E; ++I) {
				for (unsigned i = 0, e = I->second.size(); i != e; ++i)
					addChainDependency(AA, MFI, SU, I->second[i], RejectMemNodes, TrueMemOrderLatency);
			}
			adjustChainDeps(AA, MFI, SU, &ExitSU, RejectMemNodes, TrueMemOrderLatency);
			PendingLoads.clear();
			AliasMemDefs.clear();
			...
			bool MayAlias = false;
			for (SmallVector<std::pair<const Value *, bool>, 4>::iterator K = Objs.begin(), KE = Objs.end(); K != KE; ++K) {
				...
				// an existing def.
				MapVector<const Value *, SUnit *>::iterator I = ((ThisMayAlias) ? AliasMemDefs.find(V) : NonAliasMemDefs.find(V));
				MapVector<const Value *, SUnit *>::iterator IE = ((ThisMayAlias) ? AliasMemDefs.end() : NonAliasMemDefs.end());
				if (I != IE) {
				...
				MapVector<const Value *, std::vector<SUnit *> >::iterator J = ((ThisMayAlias) ? AliasMemUses.find(V) : NonAliasMemUses.find(V));
				MapVector<const Value *, std::vector<SUnit *> >::iterator JE = ((ThisMayAlias) ? AliasMemUses.end() : NonAliasMemUses.end());
				if (J != JE) {
					for (unsigned i = 0, e = J->second.size(); i != e; ++i)
						addChainDependency(AA, MFI, SU, J->second[i], RejectMemNodes, TrueMemOrderLatency, true);
					J->second.clear();
				}
			}
			if (MayAlias) {
			...
			if (MI->isInvariantLoad(AA)) {
				...
				for (SmallVector<std::pair<const Value *, bool>, 4>::iterator J = Objs.begin(), JE = Objs.end(); J != JE; ++J) {
...
class SchedDFSImpl {
	...
	struct RootData {
		...
		RootData(unsigned id) :
				NodeID(id), ParentNodeID(SchedDFSResult::InvalidSubtreeID), SubInstrCount(0) {
		}
...
public:
	SchedDFSImpl(SchedDFSResult &r) :
			R(r), SubtreeClasses(R.DFSNodeData.size()) {
	...
	bool isVisited(const SUnit *SU) const {
		return R.DFSNodeData[SU->NodeNum].SubtreeID != SchedDFSResult::InvalidSubtreeID;
	}

	/// Initialize this node's instruction count. We don't need to flag the node
	/// visited until visitPostorder because the DAG cannot have cycles.
	void visitPreorder(const SUnit *SU) {
		R.DFSNodeData[SU->NodeNum].InstrCount = SU->getInstr()->isTransient() ? 0 : 1;
	}
	...
	void visitPostorderEdge(const SDep &PredDep, const SUnit *Succ) {
		R.DFSNodeData[Succ->NodeNum].InstrCount += R.DFSNodeData[PredDep.getSUnit()->NodeNum].InstrCount;
	...
	void finalize() {
		...
		assert(SubtreeClasses.getNumClasses() == RootSet.size() && "number of roots should match trees");
		for (SparseSet<RootData>::const_iterator RI = RootSet.begin(), RE = RootSet.end(); RI != RE; ++RI) {
		...
		for (unsigned Idx = 0, End = R.DFSNodeData.size(); Idx != End; ++Idx) {
			...
			DEBUG(dbgs() << "  SU(" << Idx << ") in tree " << R.DFSNodeData[Idx].SubtreeID << '\n');
		}
		for (std::vector<std::pair<const SUnit*, const SUnit*> >::const_iterator I = ConnectionPairs.begin(), E = ConnectionPairs.end(); I != E; ++I) {
...
protected:
	...
	bool joinPredSubtree(const SDep &PredDep, const SUnit *Succ, bool CheckLimit = true) {
		...
		for (SUnit::const_succ_iterator SI = PredSU->Succs.begin(), SE = PredSU->Succs.end(); SI != SE; ++SI) {
	...
	void addConnection(unsigned FromTree, unsigned ToTree, unsigned Depth) {
		...
		do {
			SmallVectorImpl<SchedDFSResult::Connection> &Connections = R.SubtreeConnections[FromTree];
			for (SmallVectorImpl<SchedDFSResult::Connection>::iterator I = Connections.begin(), E = Connections.end(); I != E; ++I) {
				if (I->TreeID == ToTree) {
...
public:
	...
	const SUnit *getCurr() const {
		return DFSStack.back().first;
	}

	SUnit::const_pred_iterator getPred() const {
...
static bool hasDataSucc(const SUnit *SU) {
	for (SUnit::const_succ_iterator SI = SU->Succs.begin(), SE = SU->Succs.end(); SI != SE; ++SI) {
...
void SchedDFSResult::compute(ArrayRef<SUnit> SUnits) {
	...
	for (ArrayRef<SUnit>::const_iterator SI = SUnits.begin(), SE = SUnits.end(); SI != SE; ++SI) {
		const SUnit *SU = &*SI;
		...
		for (;;) {
			...
			while (DFS.getPred() != DFS.getPredEnd()) {
				...
				if (PredDep.getKind() != SDep::Data || PredDep.getSUnit()->isBoundaryNode()) {
					continue;
				}
...
void SchedDFSResult::scheduleTree(unsigned SubtreeID) {
	for (SmallVectorImpl<Connection>::const_iterator I = SubtreeConnections[SubtreeID].begin(), E = SubtreeConnections[SubtreeID].end(); I != E; ++I) {
		SubtreeConnectLevels[I->TreeID] = std::max(SubtreeConnectLevels[I->TreeID], I->Level);
		DEBUG(dbgs() << "  Tree: " << I->TreeID << " @" << SubtreeConnectLevels[I->TreeID] << '\n');
	}
}
