...
public:
	...
	enum MIFlag {
		NoFlags = 0, FrameSetup = 1 << 0,    // Instruction is used as a part of
...
private:
	...
	void setParent(MachineBasicBlock *P) {
		Parent = P;
	}
	...
	MachineInstr(MachineFunction&, const MCInstrDesc &MCID, const DebugLoc dl, bool NoImp = false);
...
public:
	...
	///
	bool isLabel() const {
		return getOpcode() == TargetOpcode::PROLOG_LABEL || getOpcode() == TargetOpcode::EH_LABEL || getOpcode() == TargetOpcode::GC_LABEL;
	}
	...
	bool isKill() const {
		return getOpcode() == TargetOpcode::KILL;
	}
	...
	bool isIdentityCopy() const {
		return isCopy() && getOperand(0).getReg() == getOperand(1).getReg() && getOperand(0).getSubReg() == getOperand(1).getSubReg();
	}
	...
	void addRegisterDefined(unsigned IncomingReg, const TargetRegisterInfo *RegInfo = 0);
	...
	bool isSafeToReMat(const TargetInstrInfo *TII, AliasAnalysis *AA, unsigned DstReg) const;
	...
	//Metadata addition to Machine instructions for describing a HyperOp Interaction Graph (HIG) for execution on REDEFINE
	void setMetadata(MDNode *Node);

	MDNode * getMetadata();
	//END of methods for metadata addition to Machine instructions for describing a HyperOp Interaction Graph (HIG) for execution on REDEFINE
...
struct MachineInstrExpressionTrait: DenseMapInfo<MachineInstr*> {
	...
	static bool isEqual(const MachineInstr* const &LHS, const MachineInstr* const &RHS) {
		if (RHS == getEmptyKey() || RHS == getTombstoneKey() || LHS == getEmptyKey() || LHS == getTombstoneKey())
			return LHS == RHS;
