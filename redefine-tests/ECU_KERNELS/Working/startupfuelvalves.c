...
*/
int *startup_fuel_valves(float current_nh, int input_i[]) {//int current_nh, int startup_time, int *fuel_main_valve_cmd, int *fuel_start_valve_cmd, int *start_valve_has_been_closed, int *t_start_valve_open, int *t_main_valve_open) {
	//float current_nh = input;
	if (input_i[0] == 0) {
		...
		input_i[1] = FUEL_MAIN_VALVE_CLOSE_CMD; //asl_close_fuel_main_valve();
		input_i[2] = FUEL_START_VALVE_CLOSE_CMD; //asl_close_fuel_startup_valve();
		...
		if (current_nh < NH_START_VALVE_OPEN) {
			...
			if (input_i[3] == 0) {
				...
				input_i[2] = FUEL_START_VALVE_OPEN_CMD; //asl_open_fuel_startup_valve();
				/* Set time stamp */
				if (input_i[4] == -1) {
		...
		if (current_nh > (NH_MAIN_RAMP_VALVE_OPEN)) {
			...
			input_i[1] = FUEL_MAIN_VALVE_OPEN_CMD; //asl_open_fuel_main_valve();
			if (input_i[5] == -1) {
...
//	redefine_out_fuel_main_valve_cmd = 
	int *output = startup_fuel_valves(redefine_in_f, redefine_in_i);//redefine_in_current_nh, redefine_in_startup_time, &redefine_in_fuel_main_valve_cmd, &redefine_in_fuel_start_valve_cmd, &redefine_in_start_valve_has_been_closed, &redefine_in_t_start_valve_open, &redefine_in_t_main_valve_open);
	...
	redefine_out_b[2] = output[3];
	redefine_out_b[3] = output[4];
	redefine_out_b[4] = output[5];
