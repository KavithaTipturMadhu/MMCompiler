...
class REDEFINEABIInfo : public ABIInfo {
  static const unsigned MinABIStackAlignInBytes = 4;
public:
  REDEFINEABIInfo(CodeGenTypes &CGT) : ABIInfo(CGT) {}
...
class REDEFINETargetCodeGenInfo : public TargetCodeGenInfo {
public:
	REDEFINETargetCodeGenInfo(CodeGenTypes &CGT)
    : TargetCodeGenInfo(new REDEFINEABIInfo(CGT)) {}
};

}

bool REDEFINEABIInfo::isCompoundType(QualType Ty) const {
  return Ty->isAnyComplexType() || isAggregateTypeForABI(Ty);
}

bool REDEFINEABIInfo::isFPArgumentType(QualType Ty) const {
  if (const BuiltinType *BT = Ty->getAs<BuiltinType>())
    switch (BT->getKind()) {
...
llvm::Value *REDEFINEABIInfo::EmitVAArg(llvm::Value *VAListAddr, QualType Ty,
                                       CodeGenFunction &CGF) const {
  llvm::Type *BP = CGF.Int8PtrTy;
  llvm::Type *BPP = CGF.Int8PtrPtrTy;
...
ABIArgInfo REDEFINEABIInfo::classifyReturnType(QualType RetTy) const {
  uint64_t Size = getContext().getTypeSize(RetTy);
...
const TargetCodeGenInfo &CodeGenModule::getTargetCodeGenInfo() {
  ...
  case llvm::Triple::riscv:
    return *(TheTargetCodeGenInfo = new RISCVTargetCodeGenInfo(Types));

  case llvm::Triple::redefine:
      return *(TheTargetCodeGenInfo = new REDEFINETargetCodeGenInfo(Types));
