...
static void getRISCVTargetCPU(const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    StringRef MCPU = A->getValue();
    CmdArgs.push_back("-target-cpu");
    CmdArgs.push_back(Args.MakeArgString(MCPU));
  }
  ...
  //feature decoding TODO:make cleaner
...
//Start REDEFINE
static void getREDEFINETargetCPU(const ArgList &Args,
                              ArgStringList &CmdArgs,
                                     const llvm::Triple &Triple) {
  // FIXME: Warn on inconsistent use of -mcpu and -mriscv
...
//  }
  ...
  //feature decoding TODO:make cleaner
//  if(MArch.startswith("REDEFINE32")) {
//    CmdArgs.push_back("-target-feature");
//    CmdArgs.push_back("+REDEFINE32");
//    CmdArgs.push_back("-target-feature");
//    CmdArgs.push_back("-rv64");
//    parseRISCVExtensions(MArch.drop_front(4), CmdArgs);
//  }else if(MArch.startswith("RV64")) {
...
void Clang::AddREDEFINETargetArgs(const ArgList &Args,
                              ArgStringList &CmdArgs) const {
  llvm::Triple Triple = getToolChain().getTriple();
  // Set the CPU based on -march= and -mcpu=.
  getREDEFINETargetCPU(Args, CmdArgs, Triple);
}
//End REDEFINE
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  case llvm::Triple::redefine:
      AddREDEFINETargetArgs(Args, CmdArgs);
      break;
...
void gnutools::Link::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  else if (ToolChain.getArch() == llvm::Triple::redefine) {
      StringRef cpu = "";
      if (Arg *A = Args.getLastArg(options::OPT_mriscv_EQ))
        cpu = A->getValue();
      if (Arg *A = Args.getLastArg(options::OPT_mcpu_EQ))
